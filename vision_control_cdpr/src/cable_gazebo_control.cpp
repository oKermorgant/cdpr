...
#include <vision_control_cdpr/cable_gazebo_control.h>
...
namespace gazebo
{
  ...
  /*void CableControlPlugin::ReadVector3(const std::string &_string, math::Vector3 &_vector)
{
  ...
  /* bool CableControlPlugin::SwitchService(std_srvs::EmptyRequest &req, std_srvs::EmptyResponse &res)
{
  ...
  void CableControlPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    // get model and name
    model_ = _model;
        ...
        if(wrench_control_)
          {
            ops = ros::SubscribeOptions::create<geometry_msgs::Wrench>(
                  ...
                  boost::bind(&CableControlPlugin::BodyCommandCallBack, this, _1),
        ...
        else
          {
            ops = ros::SubscribeOptions::create<sensor_msgs::JointState>(
                  body_command_topic, 1,
                  boost::bind(&CableControlPlugin::ThrusterCommandCallBack, this, _1),
                  ros::VoidPtr(), &callback_queue_);
          }
    ...
    if(control_joints_ && model_->GetJointCount() != 0)
      {
        ...
        ros::SubscribeOptions ops = ros::SubscribeOptions::create<sensor_msgs::JointState>(
              ...
              boost::bind(&CableControlPlugin::JointCommandCallBack, this, _1),
              ros::VoidPtr(), &callback_queue_);
    ...
    //switch_service_ = rosnode_.advertiseService<std_srvs::EmptyRequest, std_srvs::EmptyResponse>("switch", &CableControlPlugin::SwitchService, this);
    ...
    update_event_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&CableControlPlugin::Update, this));

    ros::spinOnce();
    ROS_INFO("Started Cable Control Plugin for %s.", _model->GetName().c_str());
  }

  void CableControlPlugin::Update()
  {
    // activate callbacks
    ...
    if(controller_is_running_)
      {
        ...
        if(control_joints_ && joint_command_received_ && joint_command_.velocity.size())
          {
            physics::JointPtr joint;
            for(unsigned int i=0;i<joints_.size();++i)
              {
                //cout << "Trying to write joint_setpoint.velocity for " << i << endl;
                joint = joints_[i];
                if(true || !((joint->GetAngle(0) == joint->GetUpperLimit(0) && joint_command_.velocity[i] > 0) ||
                             (joint->GetAngle(0) == joint->GetLowerLimit(0) && joint_command_.velocity[i] < 0)))
                  {
                    //joint->SetForce(0,joint_command_.effort[i] - joint->GetDamping(0) * joint->GetVelocity(0));
                    //joint->SetForce(0,joint_command_.effort[i]);
                    joint->SetVelocity(0, joint_command_.velocity[i]);
                    cout << "Velocity of " << i << " is " << joint->GetVelocity(0) << endl;
                  }
              }
          }
  ...
  void CableControlPlugin::ComputePseudoInverse(const Eigen::MatrixXd &_M, Eigen::MatrixXd &_pinv_M)
  {
  ...
  void CableControlPlugin::BodyCommandCallBack(const geometry_msgs::WrenchConstPtr &_msg)
  {
    if(!control_body_)
      return;
  ...
  void CableControlPlugin::ThrusterCommandCallBack(const sensor_msgs::JointStateConstPtr &_msg)
  {
    if(!control_body_)
      return;
